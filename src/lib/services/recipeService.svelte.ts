...
Return only valid JSON with the recipe data.`;
  ...
  /**
   * Check if current page looks like a recipe page
   */
  static isRecipePage(tabData: TabData): boolean {
    const url = tabData.content.url.toLowerCase();
    const title = tabData.content.title.toLowerCase();
    const content = tabData.content.text.toLowerCase();
    ...
    if (recipeUrlPatterns.some(pattern => pattern.test(url))) {
      return true;
    }
    ...
    if (recipeTitlePatterns.some(pattern => pattern.test(title))) {
      return true;
    }

    // Content patterns - look for recipe-specific terms
    const recipeContentPatterns = [
      /ingredients?:/,
      /instructions?:/,
      /directions?:/,
      /prep time/,
    ...
    const matches = recipeContentPatterns.filter(pattern => pattern.test(content)).length;
    return matches >= 3; // Need at least 3 recipe-related terms
  }

  /**
   * Calculate confidence score for recipe extraction
   */
  static calculateRecipeConfidence(recipe: Recipe): number {
    let score = 0;
    let maxScore = 0;
    ...
    if (recipe.ingredients.length > 0) {
      score += Math.min(40, recipe.ingredients.length * 5);
      
      // Bonus for detailed ingredients
      const detailedIngredients = recipe.ingredients.filter(ing => ing.amount && ing.unit);
      if (detailedIngredients.length > 0) {
        score += Math.min(10, detailedIngredients.length * 2);
        maxScore += 10;
      }
    }
    ...
    if (recipe.instructions.length > 0) {
      score += Math.min(30, recipe.instructions.length * 3);
    }

    // Additional metadata - bonus points
    if (recipe.nutrition) {
      score += 5;
      maxScore += 5;
    }
